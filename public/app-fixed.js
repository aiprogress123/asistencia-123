...
const getApiBase = () => {
    ...
    console.log('📅 Versión del frontend: railway-backend-live');
    
    // Para producción, usar backend desplegado en Railway
    const API_BASE = window.location.hostname === 'localhost' 
        ? 'http://localhost:3000/api'  // Mantener HTTP para desarrollo
        : 'https://asistencia-production-f894.up.railway.app/api';  // Backend en Railway
    return API_BASE;
};
    ...
    if (currentUser && userInfo) {
        userInfo.innerHTML = `
            <div class="d-flex align-items-center">
                ...
                <span class="badge bg-secondary ms-2">${getRoleDisplayName(currentUser.role)}</span>
            </div>
        `;
        console.log('🎨 UserInfo actualizado con rol:', currentUser.role);
    }
}
...
function updateUserInterface() {
    ...
    if (currentUser && currentUser.role === 'admin') {
        // Ocultar botones de registro para admin
        if (attendanceButtons) {
            attendanceButtons.style.display = 'none';
        }
        // Mostrar mensaje informativo para admin
        if (adminMessage) {
            adminMessage.style.display = 'block';
        }
    } else {
        // Mostrar botones para empleados
        if (attendanceButtons) {
            attendanceButtons.style.display = 'flex';
        }
        // Ocultar mensaje para admin
        if (adminMessage) {
...
// Función para mostrar resumen de empleados
async function showEmployeeSummary() {
    console.log('📊 Mostrando resumen de empleados...');
    
    const token = localStorage.getItem('progressToken');
    if (!token) {
        showCustomAlert('❌ Error', 'No hay token de autenticación', 'danger');
        return;
    }
    ...
    try {
        ...
        console.log('📊 Tipo de records:', typeof records);
        console.log('📊 Es array records?:', Array.isArray(records));
        
        // Validar que records sea un array
        if (!Array.isArray(records)) {
            console.error('❌ Error: records no es un array:', records);
            console.error('❌ Tipo de records:', typeof records);
            container.innerHTML = '<p class="text-muted">Error en el formato de los datos</p>';
            return;
        }
        ...
        if (!Array.isArray(employees)) {
            console.error('❌ Error: employees no es un array:', employees);
            container.innerHTML = '<p class="text-muted">Error en el formato de los datos de empleados</p>';
            return;
        }
        ...
        // Inicializar estadísticas para cada empleado
        employees.forEach(emp => {
            if (emp.role !== 'admin') {
        ...
        records.forEach(record => {
            const email = record.employee_name || record.employee_email;
            if (!employeeStats[email]) return;
            ...
            if (record.type === 'entry') {
                ...
                stats.totalExits++;
                stats.lastExit = record.timestamp;
            }
            
            stats.workedDays.add(recordDate);
        });
        ...
        Object.keys(employeeStats).forEach(email => {
            ...
            const employeeRecords = records.filter(r => 
                (r.employee_name || r.employee_email) === email
            );
            ...
            Object.values(dailyRecords).forEach(dayRecords => {
                ...
                sorted.forEach(record => {
                    if (record.type === 'entry') {
                        ...
                        totalMinutes += Math.round((exitTime - entryTime) / (1000 * 60));
                        entryTime = null;
                    }
                });
            });
            ...
            stats.regularHours = Math.min(stats.totalHours, stats.workedDays.size * 8);
            stats.overtimeHours = Math.max(0, stats.totalHours - stats.regularHours);
            stats.workedDays = stats.workedDays.size;
        });
        
        // Mostrar resumen
        displayEmployeeSummary(employeeStats);
        
    } catch (error) {
        console.error('❌ Error cargando resumen:', error);
        showCustomAlert('❌ Error', 'No se pudo cargar el resumen de empleados', 'danger');
    }
}

function displayEmployeeSummary(employeeStats) {
    ...
    if (statsArray.length === 0) {
        container.innerHTML = '<p class="text-muted">No hay empleados para mostrar</p>';
        return;
    }
    
    // Calcular totales generales
    const totalEmployees = statsArray.length;
    const totalWorkedDays = statsArray.reduce((sum, emp) => sum + emp.workedDays, 0);
    const totalHours = statsArray.reduce((sum, emp) => sum + emp.totalHours, 0);
    const totalRegularHours = statsArray.reduce((sum, emp) => sum + emp.regularHours, 0);
    const totalOvertimeHours = statsArray.reduce((sum, emp) => sum + emp.overtimeHours, 0);
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="row text-center">
                    ...
                    <div class="col-md-3">
                        <div class="stat-card">
                            <h3 class="text-success">${totalWorkedDays}</h3>
                            <p class="mb-0">Días Totales Trabajados</p>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="stat-card">
                            <h3 class="text-info">${totalRegularHours.toFixed(1)}h</h3>
                            <p class="mb-0">Horas Regulares</p>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="stat-card">
                            <h3 class="text-warning">${totalOvertimeHours.toFixed(1)}h</h3>
    ...
    statsArray.forEach(emp => {
        const lastEntry = emp.lastEntry ? new Date(emp.lastEntry).toLocaleString('es-ES') : 'N/A';
        const lastExit = emp.lastExit ? new Date(emp.lastExit).toLocaleString('es-ES') : 'N/A';
        ...
        html += `
            <tr>
                ...
                <td>
                    <span class="badge bg-${emp.role === 'coordinator' ? 'warning' : 'secondary'}">
                        ${emp.role === 'coordinator' ? 'Coordinador' : 'Empleado'}
                    </span>
                </td>
                <td class="text-center">${emp.workedDays}</td>
                <td class="text-center">${emp.totalEntries}</td>
                <td class="text-center">${emp.totalExits}</td>
                <td class="text-center">${emp.regularHours.toFixed(1)}h</td>
                <td class="text-center">${emp.overtimeHours.toFixed(1)}h</td>
                <td class="text-center"><strong>${emp.totalHours.toFixed(1)}h</strong></td>
                <td><small>${lastEntry}</small></td>
                <td><small>${lastExit}</small></td>
            </tr>
        `;
    });
    ...
    console.log('📊 Resumen de empleados mostrado');
}

function showAllAttendance() {
    console.log('🔄 Mostrando sección de todos los registros (resumen)');
    ...
    // Mostrar resumen de empleados directamente
    showEmployeeSummary();
}
...
function displayAttendanceRecords(records) {
    ...
    records.forEach(record => {
        const employeeName = record.employee_name || record.employee_email || 'Desconocido';
        ...
        employeeRecords[employeeName].records.push(record);
    });

    console.log('� Registros agrupados por empleado:', Object.keys(employeeRecords).length, 'empleados');
    ...
    Object.values(employeeRecords).forEach(employee => {
        ...
        employee.regularHours = Math.min(totalMinutes / 60, 8); // 8 horas regulares
        employee.overtimeHours = Math.max(0, (totalMinutes / 60) - 8); // Horas extras
    });
                ...
                <div class="card-header">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <small class="text-muted">
                            ${employee.regularHours.toFixed(1)}h regulares
                            ${employee.overtimeHours > 0 ? ` + ${employee.overtimeHours.toFixed(1)}h extras` : ''}
                        </small>
                        ${userRole === 'admin' ? `
                            <button class="btn btn-sm btn-outline-danger" onclick="deleteEmployeeRecords('${employee.name}')">
                    ...
                    ${employee.records.map(record => {
                        ...
                        return `
                            <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                                ...
                                <div>
                                    ${record.photo_path ? `
                                        <img src="${API_BASE.replace('/api', '')}${record.photo_path}" alt="Foto" style="width: 40px; height: 40px; object-fit: cover; border-radius: 5px; cursor: pointer;" onclick="showRecordDetails('${encodeURIComponent(JSON.stringify(record))}')">
                                    ` : `
