...
// Sistema de Asistencia Progress - Versión de Producción
...
// Log para diagnóstico de API_BASE
...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    console.log('🔑 Password:', password ? 'proporcionado' : 'vacío');
    ...
    try {
        ...
        console.log('📡 Enviando solicitud a:', loginUrl);
        ...
        showCustomAlert('❌ Error de Login', error.message, 'danger');
    } finally {
...
function updateAdminUI(userRole) {
    console.log('🔧 Actualizando UI para rol:', userRole);
    ...
    setTimeout(() => {
        ...
        adminOnlyButtons.forEach(btnId => {
            ...
            console.log(`🔍 Buscando botón ${btnId}:`, btn ? 'encontrado' : 'NO ENCONTRADO');
            
            if (btn) {
                if (userRole === 'admin') {
                    btn.style.display = 'inline-block';
                    console.log(`✅ Botón ${btnId} visible para admin`);
                } else {
                    btn.style.display = 'none';
                    console.log(`🚫 Botón ${btnId} oculto para no-admin`);
                }
            } else {
                console.error(`❌ Botón ${btnId} no encontrado en el DOM`);
            }
        });
    }, 100);
...
async function loadEmployees() {
    console.log('🔄 Cargando empleados - VERSIÓN SIMPLIFICADA');
    ...
    try {
        ...
        console.log('👥 Empleados recibidos:', employees.length);
                ...
                <tr>
                    <td colspan="7" class="text-center">
                        <div class="alert alert-danger m-2">
                            ...
                            <button onclick="loadEmployees()" class="btn btn-sm btn-danger mt-2">
...
function displayEmployeeSummary(records) {
    ...
    if (!summaryContainer) {
        console.error('❌ Contenedor employeeSummary no encontrado');
        return;
    }
    
    console.log('✅ Contenedor employeeSummary encontrado, procesando...');
    ...
    if (!records || records.length === 0) {
        summaryContainer.innerHTML = `
            <div class="col-12 text-center text-white">
                ...
                <p class="text-muted">No se encontraron registros para generar el resumen</p>
            </div>
        `;
        console.log('📋 Mostrando mensaje de sin registros');
        return;
    }
    ...
    records.forEach(record => {
        if (!employeeStats[record.employee_name]) {
            employeeStats[record.employee_name] = {
                entries: 0,
                exits: 0,
                totalHours: 0,
                overtime: 0
        ...
        if (record.type === 'entry') {
            employeeStats[record.employee_name].entries++;
        } else {
            employeeStats[record.employee_name].exits++;
        }
    });
    
    console.log('📊 Estadísticas por empleado:', employeeStats);
    
    // Generar HTML del resumen
    let summaryHtml = '';
    Object.keys(employeeStats).forEach(employeeName => {
        const stats = employeeStats[employeeName];
        const percentage = records.length > 0 ? (stats.entries / records.length * 100) : 0;
        
        summaryHtml += `
            <div class="col-md-6 col-lg-4 mb-3">
                <div class="card" style="background: rgba(30, 41, 59, 0.9); border: 1px solid var(--progress-teal);">
                    <div class="card-body text-center">
                        <h6 class="text-white mb-3">${employeeName}</h6>
                        <div class="row text-center">
                            <div class="col-6">
                                <small class="text-white-50">Entradas:</small>
                                <div class="text-success fw-bold">${stats.entries}</div>
                            </div>
                            <div class="col-6">
                                <small class="text-white-50">Salidas:</small>
                                <div class="text-danger fw-bold">${stats.exits}</div>
                            </div>
                        </div>
                        <div class="mt-3">
                            ...
                            <small class="text-white">${percentage.toFixed(1)}% del total</small>
                        </div>
                    </div>
                </div>
            </div>
...
function showAllAttendance() {
    console.log('🔄 Mostrando sección de todos los registros - VERSIÓN DEFINITIVA');
    ...
    // Cargar registros directamente - SIN DEPENDENCIAS
    loadAllRecordsDirectly();
}

// Función completamente independiente para cargar registros
async function loadAllRecordsDirectly() {
    console.log('📊 Cargando todos los registros - FUNCIÓN INDEPENDIENTE');
    ...
    try {
        ...
        console.log('🔑 Token presente:', !!localStorage.getItem('progressToken'));
        ...
        if (window.location.hostname.includes('ngrok')) {
            ...
            headers['Accept'] = 'application/json';
            headers['Cache-Control'] = 'no-cache';
            console.log('🔑 Headers ngrok activados');
        }
        
        const url = `${API_BASE}/admin/attendance`;
        ...
        // Llamar al resumen con los registros
        displayEmployeeSummary(records);
            ...
            container.innerHTML = `
                <tr>
                    <td colspan="5" class="text-center">
                        <div class="alert alert-danger m-2">
                            <i class="fas fa-exclamation-triangle me-2"></i>
                            <strong>Error al cargar registros</strong><br>
                            <small>${error.message}</small><br>
                            <button onclick="loadAllRecordsDirectly()" class="btn btn-sm btn-danger mt-2">
                                <i class="fas fa-sync me-1"></i>Reintentar
                            </button>
                        </div>
                    </td>
...
// Función para mostrar registros - INDEPENDIENTE
function displayAllRecords(records) {
    ...
    records.forEach(record => {
        ...
        // Corregir URL de la foto para entorno local
        let photoUrl = '';
        if (record.photo_path) {
            // Siempre usar localhost:3000 para fotos
            photoUrl = `http://localhost:3000/${record.photo_path}`;
        }
        
        html += `
            <tr>
                <td><span class="text-black bg-white px-2 py-1 rounded">${record.employee_name || 'N/A'}</span></td>
                <td><span class="badge bg-${typeClass}">${typeIcon} ${typeText}</span></td>
                <td><span class="text-black bg-white px-2 py-1 rounded">${dateStr}<br><small>${timeStr}</small></span></td>
                <td>
                    ${photoUrl ? 
                        `<img src="${photoUrl}" alt="Foto" style="width: 40px; height: 40px; object-fit: cover; border-radius: 5px; cursor: pointer;" onclick="showImageModal('${photoUrl}')">` : 
    ...
    container.innerHTML = html;
    console.log('✅ Registros mostrados correctamente');
}
...
async function deleteEmployee(employeeId, employeeName) {
    console.log('🗑️ Función deleteEmployee llamada con:', { employeeId, employeeName });
    
    // Obtener el usuario actual para verificar si es admin
    const currentUser = JSON.parse(localStorage.getItem('progressUser'));
    
    if (currentUser.role !== 'admin') {
        console.error('❌ Usuario no es admin:', currentUser.role);
    ...
    const confirmMessage = `⚠️ ¿Estás seguro de que quieres eliminar al empleado "${employeeName}"?\n\nEsta acción eliminará:\n• Todos sus registros de asistencia\n• Su información personal\n• No se puede deshacer`;
    console.log('🔍 Mostrando confirmación:', confirmMessage);
    ...
    try {
        console.log('📡 Enviando solicitud DELETE a:', `${API_BASE}/admin/employees/${employeeId}`);
        console.log('🔑 Token usado:', localStorage.getItem('progressToken') ? 'presente' : 'AUSENTE');
        console.log('🌐 API_BASE:', API_BASE);
